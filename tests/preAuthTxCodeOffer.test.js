...
describe('Pre-Auth TX Code Offer Routes (real controllers)', () => {
  let app;
  let globalSandbox;
  ...
  for (const route of TX_CODE_OFFER_ROUTES) {
    describe(`GET ${route.path}`, () => {
      it('returns qr, deepLink, sessionId, and txCode', async () => {
        ...
        const credentialType = 'VerifiablePortableDocumentA1SDJWT';
      ...
      it('stores expectedTxCode in the session matching the response txCode', async () => {
        ...
        expect(storedSession).to.have.property('txCodeRequired', true);
