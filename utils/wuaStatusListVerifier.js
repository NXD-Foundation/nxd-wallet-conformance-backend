/**
 * RFC004 / IETF Token Status List (draft-20) — fetch, signature verify, and bit read.
 * Structural and cryptographic checks only; no trust-anchor or WP trust-list validation.
 */
...
function fail(message, options = {}) {
  throw new WuaStatusListValidationError(
    withSpecRef(message, SPEC_REFS.RFC004, SPEC_REFS.DRAFT20),
    options
  );
}
...
async function fetchStatusListJwt(uri, options) {
  ...
  try {
    ...
    if (!response.ok) {
      fail(`${kindLabel(options.kind)} Status List Token fetch failed: HTTP ${response.status}`, {
        kind: options.kind,
        reason: "fetch_failed",
    ...
    if (buf.length > maxBytes) {
      fail(`${kindLabel(options.kind)} Status List Token response exceeds size limit`, {
        kind: options.kind,
        reason: "fetch_failed",
        statusState: "unavailable",
      });
    }
    const jwt = buf.toString("utf8").trim();
    ...
    fail(`${kindLabel(options.kind)} Status List Token fetch failed: ${error?.message || String(error)}`, {
      kind: options.kind,
      reason: "fetch_failed",
      statusState: "unavailable",
    });
