...
import { evaluateCredentialSets } from "../../utils/dcqlCore.js";
...
async function findWalletMatchesForDcqlQuery({
  ...
  for (const t of types) {
    ...
    const fallbackDocType = inferDoctypeFromStoredCredential(t, stored);
    const candidates = [];
    if (stored.multi && Array.isArray(stored.entries)) {
      for (const entry of stored.entries) {
        const token = extractCredentialString(entry?.credential);
        if (token) {
          candidates.push({ token, pickedEntry: entry });
        }
      }
    } else {
    ...
    for (const { token, pickedEntry } of candidates) {
      if (
    ...
    if (matches.length > 0 && credQuery.multiple !== true) {
      break;
    }
  }
  return matches;
...
function resolveTargetCredentialQueryIds(dcqlQuery, matchesByQueryId) {
  ...
  const evaluation = evaluateCredentialSets(dcqlQuery, Object.fromEntries([...matchedIds].map((id) => [id, true])));
  if (!evaluation.satisfied) return null;
  if (!Array.isArray(dcqlQuery?.credential_sets)) {
  ...
  if (evaluation.requiredSets.length === 0) {
    // An explicitly present all-optional credential_sets array must not turn
    // every credential query into a requirement. Present what is available.
