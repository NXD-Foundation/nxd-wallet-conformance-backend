...
describe("dcqlCredentialSelection", () => {
  ...
  describe("selectWalletCredentialTypeForDcql", () => {
    ...
    it("returns available credentials when credential_sets are all optional", async () => {
      const sd = await buildDcSdJwtForTests("eu.test.demo");
      const store = {
        demo: { credential: { credential: sd } },
      };
      const dcqlQuery = {
        credential_sets: [{ required: false, options: [["sd-id"], ["missing-mdoc"]] }],
        credentials: [
          {
            id: "sd-id",
            format: "dc+sd-jwt",
      ...
      expect(selections[0].matchedQuery.id).to.equal("sd-id");
    });

    it("allows no selections when credential_sets are all optional", async () => {
      const dcqlQuery = {
        credential_sets: [{ required: false, options: [["sd-id"]] }],
        credentials: [
          {
            id: "sd-id",
            format: "dc+sd-jwt",
    ...
    it("selects individual entries from multi-entry cached credentials when multiple=true", async () => {
      const vctWanted = "eu.webuildconsortium.helloworld.v1";
      const sd1 = await buildDcSdJwtForTests(vctWanted);
      const sd2 = await buildDcSdJwtForTests(vctWanted);
      const store = {
        batch: {
          multi: true,
          entries: [
            { credential: { credential: sd1 }, keyBinding: { publicJwk: { kty: "EC" } } },
            { credential: { credential: sd2 }, keyBinding: { publicJwk: { kty: "EC" } } },
          ],
        },
      };
      const dcqlQuery = {
      ...
      expect(selections).to.have.length(2);
      expect(selections.every((entry) => entry.selectedType === "batch")).to.equal(true);
      expect(selections.every((entry) => entry.pickedEntry?.credential)).to.equal(true);
    });
