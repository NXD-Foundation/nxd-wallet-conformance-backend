...
import { DcqlDisclosureSelectionError } from "../src/lib/sdJwtDisclosureSelection.js";
...
describe("sdJwtDisclosureSelection", () => {
  ...
  it("throws when a requested DCQL claim path is invalid", () => {
    ...
    ).to.throw(DcqlDisclosureSelectionError);
  });
  ...
  it("throws when an SD-JWT claim does not satisfy the DCQL values constraint", () => {
    ...
    ).to.throw(DcqlDisclosureSelectionError, /does not satisfy requested DCQL values constraint/);
  });
});
